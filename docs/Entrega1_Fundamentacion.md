...
**Cuellos de Botella Técnicos** | **Saturación de la base de datos compartida:** Al procesar inventarios, ventas y contabilidad en la misma base de datos relacional, las consultas pesadas en horas pico bloquean las tablas de stock (*table locking*). | **Límites estrictos de peticiones (*Rate Limiting*):** La API impone topes diarios/minuto de llamadas, lo que impide hacer consultas masivas de stock o integrar sistemas automatizados a alta velocidad. | **Rendimiento atado a servidores compartidos (*Noisy Neighbor*):** Si la nube de Alegra recibe mucha carga de otros clientes, los tiempos de respuesta del inventario y la facturación se vuelven lentos. |
| **Puntos Únicos de Falla y Riesgos** | **Caída total por acoplamiento (*SPOF*):** Si el servidor principal de Python o la base de datos PostgreSQL fallan, se cae todo el ERP (ventas, facturación y bodegas quedan inoperativas al mismo tiempo). | **Dependencia total del proveedor (*Vendor Lock-in*):** No se puede modificar la infraestructura ni la lógica interna. Si la nube de Zoho sufre un corte de servicio, la bodega queda completamente paralizada. | **Sin control de infraestructura ni backups directos:** Al ser un SaaS cerrado, no hay acceso a modificar la persistencia ni a desplegar módulos propios si la empresa requiere una regla de negocio especial. |
...
| 19/09/2026 | Andrés Felipe Gálvez Carmona | Gemini | Definición de criterios técnicos y reestructuración de la matriz comparativa de sistemas de inventario. | *Con base en lo investigado de los 3 sistemas (Odoo, Zoho, Alegra), identifica y dame criterios técnicos para hacer la comparativa.* | **Medio** | Se establecieron 10 criterios de evaluación técnica y se ajustó el lenguaje de la tabla comparativa, facilitando la defensa de la arquitectura de microservicios propia frente a soluciones monolíticas y SaaS. |
...
| 20/09/2026 | Andrés Felipe Gálvez Carmona | NotebookLM | Auditoría y validación de cumplimiento de la Entrega 1 frente al documento guía `Proyecto_ISW2`. | *Evalúa si la Entrega 1 de LogiTech Solutions cumple con todos los requerimientos de la guía Proyecto_ISW2, identifica vacíos o faltantes y dame recomendaciones para asegurar la mejor nota.* | **Medio** | Se realizó una revisión integral del documento contra la guía del docente, verificando el cumplimiento de las Secciones 1 a 5 y detectando puntos clave a reforzar antes de la entrega definitiva. |
| 24/09/2026 | Mariana Duque Giraldo | Gemini / NotebookLM | Auditoría y evaluación de coherencia de requerimientos funcionales y no funcionales frente a la guía del proyecto. | Evalúa los requerimientos funcionales y no funcionales respecto a las pautas de Proyecto_ISW2 y verifica su coherencia técnica con el caso de estudio de LogiTech Solutions S.A.S. | Medio | Se validó la alineación de los RF y RNF con la rúbrica del curso, confirmando la consistencia técnica entre el alcance del caso de estudio, el modelo de 2 bodegas y la arquitectura de microservicios. |
